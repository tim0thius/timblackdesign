...
// show()
            ...
            private function add_submenu( $page_parent, $page_title, $menu_title, $page_permissions, $page_slug ) {
                ...
                if ( isset ( $test[ $page_parent ] ) ) {
                    ...
                    if ( true == $addMenu ) {
						...
						$submenu_type = 'add_submenu';
                        $this->page = call_user_func( $submenu_type . '_page', $page_parent, $page_title, $menu_title, $page_permissions, $page_slug, array(
                            &$this,
                            'generate_panel'
                        ) );
                    }
            ...
            public function _options_page() {
                ...
                if ( $this->args['menu_type'] == 'hidden' ) {
					...
					$addmenu_type = 'add_menu';
                    // Theme-Check notice is displayed for WP.org theme devs, informing them to NOT use this.
                    $this->page = call_user_func( $addmenu_type . '_page', $this->args['page_title'], $this->args['menu_title'], $this->args['page_permissions'], $this->args['page_slug'], array(
                        &$this,
                        'generate_panel'
                    ...
                    if ( true === $this->args['allow_sub_menu'] ) {
                        if ( ! isset ( $section['type'] ) || $section['type'] != 'divide' ) {
                            foreach ( $this->sections as $k => $section ) {
                                ...
                                // ONLY for non-wp.org themes OR plugins. Theme-Check alert shown if used and IS theme.
								$submenu_type = 'add_submenu';
                                call_user_func( $submenu_type . '_page', $this->args['page_slug'], $section['title'], $section['title'], $this->args['page_permissions'], $this->args['page_slug'] . '&tab=' . $k,
            ...
            public function _enqueue_output() {
                ...
                if ( ! empty ( $this->typography ) && ! empty ( $this->typography ) && filter_var( $this->args['output'], FILTER_VALIDATE_BOOLEAN ) ) {
                    ...
                    if ( $this->args['async_typography'] && ! empty ( $this->typography ) ) {
                        ...
                        wp_register_style( 'google-fonts-' . $this->args['opt_name'], $protocol . $typography->makeGoogleWebfontLink( $this->typography ), '', $version );
                        wp_enqueue_style( 'google-fonts-' . $this->args['opt_name'] );
                    }
                }
            }
// _enqueue_output()
